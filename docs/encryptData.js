...
const filepath = './data/encryptedFacebookFriendsData.json'
// All fields not in 'fieldsToEncrypt' will be removed
...
let rawdata = fs.readFileSync('./private/FacebookFriendsData.json');
...
getPassphrase()
.then(p => passphrase = p)
.then(passphrase => encryptData(passphrase))
.then(e => encrypted = e)
// .then(e => {console.log('Encrypted:', e); return e})
// .then(encrypted => decryptData(encrypted, passphrase))
// .then(d => {console.log('Decrypted:', d); return d})
.then(() => fs.writeFileSync(filepath, JSON.stringify(encrypted)))
...
async function getPassphrase() {
  return new Promise((resolve, reject) => {
    prompt.get([{
      // The fist input text is assigned to secretPassphrase variable.
      name: 'secretPassphrase',
      // The secretPassphrase must match below regular expression.
      validator: /^[a-zA-Z\s\-]+$/,
      // If secretPassphrase is not valid then prompt below message.
      warning: 'secretPassphrase is not valid, it can only contains letters, spaces, or dashes',
      hidden: true
    }], function onInput(err, result) {
      if (err) reject(err)
      resolve(result.secretPassphrase)
    })
  })
}

function encryptData(passphrase) {
  let encrypted = encryptFields(data, fieldsToEncrypt, passphrase)

  if (!data.friends) return encrypted
  encrypted.friends = []
  data.friends.forEach((friend, i) => {
    let salt = Tea.encrypt(friend[fieldToUseAsSalt], passphrase)
    let saltedpassphrase = salt.slice(0,2) + passphrase.slice(0,14)
    encryptedFriend = encryptFields(friend, fieldsToEncrypt, saltedpassphrase)
    encryptedFriend[fieldToUseAsSalt] = salt
    encrypted.friends[i] = encryptedFriend
  })

  if (!data.links) return encrypted
  encrypted.links = []
  data.links.forEach((link, i) => {
    encrypted.links[i] = encryptFields(link, fieldsToEncrypt, passphrase)
  })

  encrypted.readable = Tea.encrypt('true',passphrase)
...
function encryptFields(data, fields, key) {
  ...
  for (let field of fields) {
    if (data[field]) {
      encryptedData[field] = Tea.encrypt(data[field], key)
    }
  }
